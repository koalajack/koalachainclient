...
BOOL CRPDlg::OnInitDialog()
{
	...
	strTitle += theApp.m_bChinese ? _T("请求付款到") : _T("Request payment to");
	strTitle += m_strRecvAddr;
	SetWindowText(strTitle);

	strInfo += theApp.m_bChinese ? "付款信息\n" : "Payment info\n";
	...
	if(m_strRecvAddr.Compare(""))
	{
		...
		strAddr += theApp.m_bChinese ? "地址:" : "Address";
	...
	{
		string strAccount;
		strAccount += theApp.m_bChinese ? "金额:" : "Amount";
	...
	if(m_strRecvLabel.Compare(""))
	{
		...
		strLabel += theApp.m_bChinese ? "标签:" : "Label";
	...
	if(m_strRecvMessage.Compare(""))
	{
		...
		strMessage += theApp.m_bChinese ? "消息:" : "Message";
...
{	
	...
	memDC.FillSolidRect(&rect,RGB(255,255,255));//这些时绘图操作，随便你^_^
