...
BOOL CStartProgress::OnInitDialog()
{
	...
	SetWindowPos(NULL , x/2 - 175 , y/2 - 50  , 280 ,330 , SWP_SHOWWINDOW) ;
	CenterWindow();

	if ( NULL == m_ProgressWnd ) {
		m_ProgressWnd = new CGIFControl ;
		m_ProgressWnd->Create(_T("") , WS_CHILD | SS_OWNERDRAW | WS_VISIBLE | SS_NOTIFY , CRect(0,0,280,300) , this, 121 ) ;
	...
	m_progress.MoveWindow(0, 300, 280, 30);
	m_progress.SetBarBkColor(RGB(255, 255, 255));
	m_progress.SetBarColor(RGB(183, 225, 243));
		...
		if( m_ProgressWnd->GetSafeHwnd() ) {
			if( TRUE == bState ) {
				...
				if(theApp.m_bChinese)
					gifpath =strprintf("%s\\gif\\init-chinese.gif\0",theApp.m_strInsPath);
				else
					gifpath =strprintf("%s\\gif\\init-english.gif\0",theApp.m_strInsPath);
				
				if( TRUE == ((CGIFControl*)m_ProgressWnd)->Load((char*)gifpath.c_str())) {
					CRect rc ;
					GetClientRect( rc ) ;
					Invalidate() ;
					m_ProgressWnd->SetWindowPos( NULL , /*(rc.Width()*3)/4+10 , (rc.Height()*9)/32 , 0 , 0 ,*/0, 0, 280, 300, \
