...
BOOL CSqliteDeal::InitializationDB(){
	...
	//创建请求付款的历史记录表LBF2016/04/19
	strCondition = "type='table' and name='t_payment_request'";
	if(!GetTableCountItem(strTableName, strCondition))
	{
		string createSQL= "CREATE TABLE t_payment_request(DataTime INT, Address TEXT,Tabel TEXT, Message TEXT, Amount double)";
		if(!ExcuteSQL(pDBConn, NULL, createSQL, NULL))
		{
			LogPrint("INFO", "Create table t_app_transaction failed\n");
			return FALSE;
...
//获取一个请求历史
int CallGetRPItem(void *para, int n_column, char ** column_value, char ** column_name)
{
	uistruct::LISTADDRHISTORY_t *pAddr = (uistruct::LISTADDRHISTORY_t *)para;
	if(NULL == column_value[0])
		return -1;
	if(n_column != 5)
		return -1;

	pAddr->datetime = atoi(column_value[0]);
	
	pAddr->address = strprintf("%s",column_value[1]);

	pAddr->label = strprintf("%s",column_value[2]);

	pAddr->message = strprintf("%s",column_value[3]);

	pAddr->fMoney = (INT64)(strtod (column_value[4], NULL));
...
//获取请求付款历史列表
int CallPaymentRequestList(void *para, int n_column, char ** column_value, char ** column_name)			//lbf
{
	map<string,uistruct::LISTADDRHISTORY_t> *pListInfo = (map<string,uistruct::LISTADDRHISTORY_t> *)para;
	if(n_column != 5)
		return -1;

	uistruct::LISTADDRHISTORY_t listdata;

	if(CallGetRPItem(&listdata, n_column, column_value, column_name) < 0 )
		return -1;
...
int CSqliteDeal::GetRPHistoryList(const string &strCondition, map<string,uistruct::LISTADDRHISTORY_t> *pListInfo)		//lbf
{
	sqlite3 ** pDBConn = GetDBConnect();
	string strSQL("");
	strSQL = "SELECT * FROM t_payment_request";
	ExcuteSQL(pDBConn, &CallPaymentRequestList, strSQL, (void *)pListInfo);
	return 0;
}
