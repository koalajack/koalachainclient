...
void COutGifDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_EXIT, m_staExit);
}
...
{/*
	if( NULL != m_ProgressWnd ) {
		if( m_ProgressWnd->GetSafeHwnd() ) {
			if( TRUE == bState ) {
				if( TRUE == ((CGIFControl*)m_ProgressWnd)->Load((char*)theApp.m_strProgressOutGifFile.c_str()) ) {
					CRect rc ;
					GetClientRect( rc ) ;
					Invalidate() ;
...
BOOL COutGifDlg::OnInitDialog()
{
	...
	m_staExit.SetWindowText(theApp.m_bChinese ? "                 程序正在关闭，请稍后..." : "The program is closing, please wait later...");
	m_staExit.SetFont(100, _T("微软雅黑"));
