...
void CJsonConfigHelp::Init()
{
		...
		memset(&m_appbalance, 0, sizeof(CAppBalance));
...
void CJsonConfigHelp::ReadJsonConfig(const string& strFilePath)
{
	...
	if (reader.parse(ifs, root,false))
	{
		...
		pos = strTemp.find("appbalance");
		if (pos >0)
		{
			ReadAppBalanceCfgData(root);
		}else{
			//UiFun::MessageBoxEx(_T("appbalance Œ¥≈‰÷√") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(0);
		}

		pos = strTemp.find("language");
		if (pos >0)
		{
			ReadLanguageCfgData(root);
		}else{
			//UiFun::MessageBoxEx(_T("language Œ¥≈‰÷√") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(0);
		}
		...
		if (pos >0)
		{
			...
			//UiFun::MessageBoxEx(_T("closeconf Œ¥≈‰÷√") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(0);
		}

		pos = strTemp.find("paramconf");
		if (pos >0)
		{
			ReadParamCfgData(root);
		}else{
			//UiFun::MessageBoxEx(_T("paramconf Œ¥≈‰÷√") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(0);
		}
		...
		if (pos >0)
		{
			...
			//UiFun::MessageBoxEx(_T("listapp Œ¥≈‰÷√") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(0);
		}

		pos = strTemp.find("businessstep");
		if (pos >0)
		{
			ReadBussinessStep(root);
		}else{
			//::MessageBoxEx(_T("businessstep Œ¥≈‰÷√") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(0);
		}
...
void CJsonConfigHelp::ReadLanguageCfgData(const Json::Value& root)
{
	Json::Value languageCfg = root["language"];
	ASSERT(!languageCfg.isNull());	
	string strName = languageCfg["name"].asString();
	if(strName == "chinese") {
		m_bChinese = true;
	} else {
		m_bChinese = false;
...
void CJsonConfigHelp::ReadScriptCfgData(const Json::Value& root){
	...
	m_Scriptid.m_strFreeze5MonthAppId = script["Freeze5"].asString();
	m_Scriptid.m_strFreeze10MonthAppId = script["Freeze10"].asString();
	m_Scriptid.m_strFreeze20MonthAppId = script["Freeze20"].asString();
}

void CJsonConfigHelp::ReadAppBalanceCfgData(const Json::Value &root)
{
	Json::Value balanceCfg = root["appbalance"];
	ASSERT(!balanceCfg.isNull());
	m_appbalance.WithdrawFee = balanceCfg["withdrawfee"].asInt64();
}

void CJsonConfigHelp::GetAppBalance(CAppBalance& appBalance)
{
	appBalance = m_appbalance;
}
...
bool CJsonConfigHelp::IsChineseLang() {
	return m_bChinese;
}
...
void CJsonConfigHelp::ReadNetParmCfgData(const Json::Value& root){
	...
	ASSERT(!netparam["server_ip"].isNull());
	Json::Value serverIpValue = netparam["server_ip"];
	m_NetParam.server_ip = netparam["server_ip"].asString();
	m_NetParam.rpc_port= netparam["rpc_port"].asString();
	m_NetParam.server_ui_port =netparam["server_ui_port"].asString();
	m_NetParam.rpc_user =  netparam["rpc_user"].asString();
	m_NetParam.rpc_password = netparam["rpc_password"].asString();
}
...
void CJsonConfigHelp::ReadParamCfgData(const Json::Value &root)
{
	Json::Value paramconfig = root["paramconf"];
	ASSERT(!paramconfig.isNull());
	m_bIsAdmin = paramconfig["admin"].asInt();
	m_strCurAdminAddress = paramconfig["admin_address"].asString();
	m_strCurEntAddress = paramconfig["ent_address"].asString();
	m_strAdminShortAddress = paramconfig["admin_short_address"].asString();
}

void CJsonConfigHelp::GetCurAdminAddress(string& strAdminAddress)
{
	strAdminAddress = m_strCurAdminAddress;
}

void CJsonConfigHelp::GetCurEntAddress(string& strEntAddress)
{
	strEntAddress = m_strCurEntAddress;
}

void CJsonConfigHelp::GetAdminCfg(int &isAdmin)
{
	isAdmin = m_bIsAdmin;
}

void CJsonConfigHelp::GetAdminShortAddress(string& strAdminShortAddress)
{
	strAdminShortAddress = m_strAdminShortAddress;
}
...
void CJsonConfigHelp::ModifyAppFeeCfgData( Json::Value& root,const CString &LeaderKey,const CString &Key,const CString &KeyValue){
	...
	if (pos > 0)
	{
		...
		CString strTemp =_T("");
		strTemp.Format(theApp.m_bChinese ? _T("%s %s  Œ¥≈‰÷√") : _T("%s %s  Not configured"),LeaderKey,Key);
...
void CJsonConfigHelp::ReadBussinessStep(const Json::Value &root)
{
	Json::Value bussparam = root["businessstep"];
	ASSERT(!bussparam.isNull());
	m_bussinessStep.ConfigStep = bussparam["Configstep"].asInt64();
	m_bussinessStep.ModifyStep= bussparam["Modifystep"].asInt64();
	m_bussinessStep.RechargeStep =bussparam["Rechargestep"].asInt64();
	m_bussinessStep.WithdrawStep =  bussparam["Withdrawstep"].asInt64();
}

void CJsonConfigHelp::GetBussinessStep(CBussinessStepCfg &bussStep)
{
	bussStep = m_bussinessStep;
}
