...
BEGIN_MESSAGE_MAP(CKoalaApp, CWinApp)
...
CKoalaApp::CKoalaApp()
{
	...
	m_strServerCfgFileName = "koalachain.conf";  //服务器配置文件名
...
//TODO: call AfxInitRichEdit2() to initialize richedit2 library.
	...
	theCrashDumper.Enable("Koala", false);
...
BOOL CKoalaApp::RunOnlyOneApp()
{
	HANDLE hMutex = CreateMutex(NULL, FALSE, _T("KoalaClient"));
	// 检查错误代码
	if (GetLastError() == ERROR_ALREADY_EXISTS) 
	{
		...
		if(theApp.m_bChinese)
			...
			::MessageBoxEx(NULL, _T("The program is already running") ,_T("tip"), MB_OK, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US));
		exit(1);  
...
void CKoalaApp::GetMoFilename( string & path , string & filename ) 
{
...
BOOL CKoalaApp::CreateMaintainThrd() 
{
	hMtThrd		= NULL ;
	nMtThrdID	= 0  ;
	...
	if( INVALID_HANDLE_VALUE == hMtThrd ) {
		if(theApp.m_bChinese)
			...
			::MessageBoxEx( NULL , "Maintenance thread creation failed!" , "Error" , MB_ICONERROR, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US) ) ;
...
UINT __stdcall CKoalaApp::MtProc(LPVOID pParam)
{
	MSG	msg , msgcpy ;
	PeekMessage(&msg, NULL, WM_USER, WM_USER, PM_NOREMOVE) ;

	if(!SetEvent( ((CKoalaApp*)pParam)->hMtStartEvent ) ) {
		return 1;
	}
	...
	{	
		...
		if ( MsgReturn )
		{
			...
			case MSG_USER_MOBILE_STATEUS:
				{
					::PostThreadMessage( ((CKoalaApp*)pParam)->GetMtHthrdId() , MSG_USER_MOBILE_TIP , msgcpy.wParam , msgcpy.lParam ) ;
				}
				break;
			case MSG_USER_QUITTHREAD:
				{
					std::vector< sThrd >::iterator it ;
					CKoalaApp* pApp = ((CKoalaApp*)pParam) ;
			...
			((CKoalaApp*)pParam)->DispatchMsg( ((CKoalaApp*)pParam)->GetMtHthrdId() , msgcpy.message , msgcpy.wParam , msgcpy.lParam ) ;
		}
	}
}
void CKoalaApp::RegThrdToMtThrd( HANDLE hThread , unsigned int hThrdId )
{
...
void CKoalaApp::UnRegThrdToMtThrd( HANDLE hThread , unsigned int hThrdId ) 
{
	if( NULL == hThrdId || NULL == hThread ) {
		return ;
	}
...
void CKoalaApp::SubscribeMsg( DWORD nThreadId , HWND hWnd, UINT msg , void * relMsgMem )
{
	if( NULL == hWnd ) {
		return ;
	}
...
void CKoalaApp::UnSubscribeMsg( DWORD nThreadId , HWND hWnd, UINT msg )
{
	if( NULL == hWnd ) {
		return ;
	}
...
void CKoalaApp::DispatchMsg( unsigned int threadID , UINT msg , WPARAM wParam , LPARAM lParam ) 
{
	BOOL flag = FALSE ;
	std::vector<MSG_NOTIFY>::iterator it ;
...
void CKoalaApp::StartSeverProcess(const string& strdir)
{
	CloseProcess(_T("koalachaind.exe"));
	...
	string startBat = strprintf("koalachaind.exe -datadir=%s -ui=1", strdir);
	if(m_bReIndexServer)
	...
	if(!CreateProcessA(NULL,(LPSTR)(startBat.c_str()),NULL,NULL,FALSE,0,NULL,NULL,&si,&sever_pi)){
		...
		if(theApp.m_bChinese)
			...
			::MessageBoxEx(NULL, _T("Open server-side program failed\n"), _T("tip"), MB_OK, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US));
		exit(1);  
	}
...
void CKoalaApp::CloseProcess(const string& exename)
{
	...
	while(TRUE) 
	{
		...
		if(SnapShot!=NULL) {
			...
			while(Status)
			{
				...
				//strlwr(pData);
				for(int i=0;i<3;i++)
					if(!strcmp(exename.c_str(),str.GetBuffer(0))){
...
void CKoalaApp::OnCheckSeverfile()
{
	string strSeverPath = strprintf("%s\\koalachaind.exe", m_strInsPath);
	...
	strSeverPath = strprintf("%s\\temp\\koalachaind.exe",m_strInsPath);
	...
	//if( (_access( strSeverPath.c_str(), 0 )) == -1 )
	//{
	//	UiFun::MessageBoxEx(_T("koala 文件不存在,请重新下载\r\n") , _T("Error") ,MFB_OK|MFB_ERROR );
	//	exit(1);
	//}
}

//校验路径合法性
void CKoalaApp::CheckPathValid(const string& strDir)
{
	BOOL bExist = FALSE;
...
void CKoalaApp::StartWorkThread() 
{
...
UINT __stdcall CKoalaApp::ReceiveMessageFromServer(LPVOID pParam)
{
	bool bReConnect(false);
	int nMaxbufferLen(10*1024-1);
	CKoalaApp * pKoalaApp  = (CKoalaApp*)pParam ;
	if ( NULL != pKoalaApp ) {
		while (true)
		{
			Sleep(100);
			...
			if(bReConnect) {
				...
				CSynchronousSocket te;
				SOCKET nSocket = te.OnblockConnnect((char*)pKoalaApp->m_severIp.c_str(),atoi(pKoalaApp->m_uiPort.c_str()));
				if( INVALID_SOCKET != nSocket) {
					pKoalaApp->m_uiConnSocket = nSocket;
					bReConnect = false;
				...
				if ( INVALID_SOCKET != pKoalaApp->m_uiConnSocket ) {
					char  cRecvbuffer[10*1024];
					int nRecLen = recv( pKoalaApp->m_uiConnSocket , cRecvbuffer , nMaxbufferLen , 0);
					...
					if ( nRecLen > 0 ) {
						...
						TRACE("\n");*/
						if(!pKoalaApp->m_noUiMsgBuffer.AddBytesToBuffer(cRecvbuffer, nRecLen)) {
							if (INVALID_SOCKET != pKoalaApp->m_uiConnSocket)
							{
								closesocket(pKoalaApp->m_uiConnSocket);
								pKoalaApp->m_uiConnSocket = INVALID_SOCKET;
							}
							pKoalaApp->m_noUiMsgBuffer.ClearBuffer();
							bReConnect = true;
						}
					}
					else if(nRecLen == 0) 
...
bool ProcessMsgJson(Json::Value &msgValue, CKoalaApp* pApp) 
{
...
UINT __stdcall CKoalaApp::PraseJsonToMessage(LPVOID pParam)
{
	CKoalaApp * pKoalaCoinApp  = (CKoalaApp*)pParam;
	if ( NULL != pKoalaCoinApp ) {
		while (true)
		{
		    Sleep(100); 
			if (theApp.m_bIsCloseAppFlag && !pKoalaCoinApp->m_noUiMsgBuffer.HaveNoUiMsg())
			{
				return 1;
			}
			while(pKoalaCoinApp->m_noUiMsgBuffer.HaveNoUiMsg()) {
				string strMsg;
				pKoalaCoinApp->m_noUiMsgBuffer.GetNoUiMsg(strMsg);
				Json::Reader reader;  
				...
				if (!reader.parse(strMsg, jsonValue)) 
					continue;
				ProcessMsgJson(jsonValue, pKoalaCoinApp);
			}
		}
	}
	return 1;
...
bool CKoalaApp::Update()
{
			...
			//if(lResult == 0xFFFFFFFF) return false;
			CString sMsg;
			sMsg.Format(theApp.m_bChinese ? _T("检查有%d文件需要更新,现在是否要更新?") : _T("Check that there are %d files that need to be updated and are now updated?"),lResult);
			
		/*	if ( IDYES != UiFun::MessageBoxEx(sMsg, UiFun::UI_LoadString("COMM_MODULE" , "COMM_TIP" ,1) , MFB_YESNO|MFB_TIP ) ){
				((ChuobiDlg*)(pApp->m_pMainWnd))->ClosWalletWind();
				return false;
			}*/

			if(m_pMainWnd != NULL)
				SendMessage(((CKoalaDlg*)(m_pMainWnd))->m_hWnd,MSG_USER_CLOSEPROCESS,NULL,NULL);
			CloseProcess("koalachaind.exe");
			DWORD exc;
			...
			ShellExecuteEx(&ShRun);
			CloseHandle(ShRun.hProcess);
			CloseProcess("Koala.exe");
			return true;
}


UINT __stdcall CKoalaApp::CheckUpdate(LPVOID lpParam){

	CKoalaApp * pApp  = (CKoalaApp*)lpParam; 
...
UINT __stdcall CKoalaApp::ProcessMessage(LPVOID pParam) {
	CKoalaApp * pApp  = (CKoalaApp*)pParam ;
	...
	while ( true)
	{
		...
		CKoalaDlg *pAppDlg = (CKoalaDlg*)(pApp->m_pMainWnd);
		if ((pAppDlg == NULL || !theApp.m_bIsDlgCreateFinished) && Postmsg.GetUItype() != MSG_USER_STARTPROCESS_UI&&Postmsg.GetDatatype() !=WM_SYNC_TRANSACTION){
			if(Postmsg.GetUItype() == MSG_USER_UP_PROGRESS && Postmsg.GetDatatype() == WM_CONNECTNET) 
			{
...
void CKoalaApp::SendUIMessage(int message, string data)
{
...
int CKoalaApp::SendPostThread(DWORD msgtype)
{
	CKoalaDlg *pDlg = (CKoalaDlg*)(((CKoalaApp*)this)->m_pMainWnd) ;
	if (pDlg == NULL)
	{
		return 0 ;
	}
...
void CKoalaApp::GetMainDlgStruct()
{
...
void CKoalaApp::InsertAppTransaction(string txdetail)
{
...
void  CKoalaApp::ParseUIConfigFile(const string& strExeDir){
	string configpath = "";
	...
	if (PathFileExistsA(configpath.c_str()))
	{
		...
		*/
		...
		CJsonConfigHelp::getInstance()->GetAdminCfg(m_bIsAdmin);
		CJsonConfigHelp::getInstance()->GetCurAdminAddress(m_strCurAdminAddress);
		CJsonConfigHelp::getInstance()->GetCurEntAddress(m_strCurEntAddress);
		CJsonConfigHelp::getInstance()->GetAdminShortAddress(m_strCurShortAddress);
		CJsonConfigHelp::getInstance()->GetBussinessStep(m_bussStep);

		m_bChinese = CJsonConfigHelp::getInstance()->IsChineseLang();

		string strPath = theApp.m_strInsPath+"\\type.ini";
		int nType = GetPrivateProfileInt("COMMON", "TYPE", 2, strPath.c_str()); 
		m_bIsAdmin = nType;
